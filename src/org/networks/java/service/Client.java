...
public class Client implements Runnable {
    ...
    private void processMessage() throws IOException {
        ...
        Constants.MessageType messageType = getMessageValue(msgStream.read1ByteMsgType());
    ...
    private void readBitFieldMsg(int messageLength) throws IOException {
        ...
        if (peer.getPieceRequestIndex(neighborPeerInfo.getPeerId()) != -1) {
            Message msg = new Message(INTERESTED, null);
            msgStreamQueue.add(new Message(INTERESTED, null));
        }
    }
    ...
    private void sendMsg(Message msg) throws IOException {
        int msgLen = msg.getMessagePacket() != null ? msg.getMessagePacket().length + 1 : 1;

        ByteBuffer buffer = ByteBuffer.allocate(msgLen + 4);
        buffer.putInt(msgLen);
        buffer.put((byte) msg.getMessageType().ordinal());

        if (msg.getMessagePacket() != null)
            buffer.put(msg.getMessagePacket());
        msgStream.getOutputStream().write(buffer.array());
    }
